...
class CategoryHorizontalCollectionViewCell: UICollectionViewCell {
    ...
    var subCategory:SubCategory? {
        didSet {
            if let subCategory = self.subCategory, let url = subCategory.imageURL, subCategory.imageURL!.characters.count > 0 {
                ...
                self.imgView.sd_setImage(with: URL(string: url), placeholderImage: UIImage(named: "horizontal_subcategory_image_placeholder"), options: .retryFailed, progress: { (i, j) in
